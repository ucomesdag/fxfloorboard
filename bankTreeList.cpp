...
QTreeWidget* bankTreeList::newTreeList()
{
    ...
    for (int a=1; a<=35; a++)
    {
        ...
        bankRange->setText(0, QString("Bank ").append(QString::number(a, 10)).append(" - ").append(QString::number(a+4, 10)) );
...
void bankTreeList::connectedSignal()
{
    ...
    if(this->openPatchTreeItems.size() != 0 && sysxIO->deviceReady() && sysxIO->isConnected())
    {
        ...
        std::sort(this->currentPatchTreeItems.begin(), this->currentPatchTreeItems.end());
